        ...
        @foreach ($functs as $funct)
            ...
            <div class="hstack mb-3">
                ...
                @elseif (($approval && ($approval->superior1_status == 1 && $approval->superior2_status == 1)) )
                    <a href="/print/{{ 'standard' }}?userType=faculty" target="_blank" class="ms-auto btn icon btn-primary" title="Print IPCR">
            ...
            @foreach ($funct->outputs as $output)
                @if ($output->user_id == Auth::user()->id && $output->type == 'ipcr' && $output->duration_id == $duration->id && $output->user_type == 'faculty')
                    <div class="card">
                        ...
                        @forelse ($output->suboutputs as $suboutput)
                            @if ($suboutput->user_id == Auth::user()->id && $suboutput->type == 'ipcr' && $suboutput->duration_id == $duration->id && $output->user_type == 'faculty')
                                <div class="card-body">
                                    <h6>{{ $suboutput->suboutput }}</h6>
                                </div>
                                <div class="card-body">
